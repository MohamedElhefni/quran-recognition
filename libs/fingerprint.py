...
def generate_hashes(peaks, fan_value=DEFAULT_FAN_VALUE):
    if PEAK_SORT:
      peaksSet = set(peaks)
      peaksSet = sorted(peaksSet, key=lambda tup: tup[1])
      peaksLen = len(peaksSet)
      # peaks.sort(key=itemgetter(1))
      # peaks.sort(key=itemgetter(1))
      # sorted(peaks, key=itemgetter(1))
      # peaksLen = len(list(peaks))
      # print(len(list(peaks)))
      # print(len(peaks[0]))
    # bruteforce all peaks
    for i in range(peaksLen):
      for j in range(1, fan_value):
        if (i + j) < peaksLen:

          # take current & next peak frequency value
          freq1 = peaksSet[i][IDX_FREQ_I]
          freq2 = peaksSet[i + j][IDX_FREQ_I]
          # take current & next -peak time offset
          t1 = peaksSet[i][IDX_TIME_J]
          t2 = peaksSet[i + j][IDX_TIME_J]
          # get diff of time offsets
          t_delta = t2 - t1
          text = f"{freq1}|{freq2}|{t_delta}"
          
          # t_delta = str(t_delta).encode('utf-8', 'ignore')
